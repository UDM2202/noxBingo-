...
import type { BingoCard } from '../game'
interface VictoryOverlayProps {
  ...
  playerName?: string
  // On-chain claim flow (multiplayer winners only)
  onClaimPrize?: () => void
  claiming?: boolean
...
function VictoryOverlay({
  ...
  // Only show the on-chain claim UI for the multiplayer winner of an
  // actual bingo (not the nox-bonus-only case, which isn't wired to
  // the vault claim instruction).
  const showClaim = isMultiplayerWinner && hasBingo && !!onClaimPrize
  useEffect(() => {
    ...
    return (
      <motion.div
        ...
        style={{
          ...
          gap: '16px',
          background:
            'radial-gradient(ellipse at center, rgba(11,11,69,0.92), rgba(11,11,69,0.98))',
        }}
      >
        <p
          style={{
        ...
        >
          ...
          {isWinner ? ' ' + trophyEmoji : ''}
          {currentViewCard === bonusCardIndex && !hasBingo ? ' ' + starEmoji : ''}
        </p>
        <div
          style={{
                    ...
                    style={{
                      ...
                      backgroundColor: isFree
                        ...
                        : isMarked
                          ? 'rgba(0,229,255,0.12)'
                          : 'transparent',
                      color: isFree ? '#FFD700' : isMarked ? '#00E5FF' : 'rgba(255,255,255,0.5)',
                      border:
                        isNox && !card.noxHit
                          ? '2px solid rgba(255,215,0,0.4)'
                          : '1px solid rgba(255,255,255,0.05)',
                      boxShadow: isNox && card.noxHit ? '0 0 12px rgba(255,215,0,0.5)' : 'none',
                    }}
                  >
        ...
        <div style={{ display: 'flex', gap: '16px', marginTop: '16px' }}>
          ...
          <button
            onClick={onBackToLobby || handlePlayAgain}
            style={{
              padding: '10px 24px',
  ...
  return (
    ...
    >
        ...
        return (
          <motion.div
            ...
            style={{
              ...
              height: '8px',
              borderRadius: '50%',
              background: i % 3 === 0 ? '#FFD700' : i % 3 === 1 ? '#00F0FF' : '#00FF88',
            }}
      ...
      >
        ...
        >
          {isMultiplayerLoser
            ? 'Better luck next time, ' + (playerName || 'Player')
            : isNoWinner
              ...
              : isDoubleWin
                ...
                : hasBingo
                  ? isMultiplayerWinner
                    ? 'You won! Card ' + ((winningCardIndex ?? 0) + 1)
          ...
          >
            {hasBingo && (
              <motion.p
                ...
                style={{ fontSize: '24px', fontWeight: 700, color: '#FFD700' }}
              >
                +100 NOX
              </motion.p>
            )}
            {hasBonus && (
              <motion.p
                ...
                style={{ fontSize: '18px', fontWeight: 600, color: '#00E5FF', marginTop: '4px' }}
              >
                +25 NOX Bonus
              </motion.p>
            )}
        ...
        {showClaim && (
          ...
          >
            {claimSignature ? (
              ...
              <>
                {payoutError ? (
                  <p style={{ color: '#FF6464', fontSize: '13px', marginBottom: '10px' }}>
                    {payoutError}
                  </p>
                ) : (
                  <p style={{ color: '#8B8BD4', fontSize: '13px', marginBottom: '10px' }}>
                    Your prize is waiting in the on-chain vault. Claim it to your wallet.
                  </p>
                )}
                <motion.button
                ...
                >
                  {claiming ? 'Claiming…' : coinEmoji + ' Claim Prize'}
                </motion.button>
                {claimError && (
                  <p style={{ color: '#FF6464', fontSize: '12px', marginTop: '8px' }}>
                    {claimError}
                  </p>
                )}
              </>
